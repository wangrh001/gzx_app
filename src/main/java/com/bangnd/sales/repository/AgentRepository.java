...
public interface AgentRepository extends JpaRepository<Agent, Long>, JpaSpecificationExecutor {
    ...
    public List<Agent> findByStateNot(@Param(value = "status") int status);
    ...
    @Query("select a from Agent a where a.ddUserName=:ddUserName and state<> 100")
    public Agent getAgentByDdUserName(@Param(value = "ddUserName") String ddUserName);
}